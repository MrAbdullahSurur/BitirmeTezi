...
public class SoundEffectManager : MonoBehaviour
{
    ...
    public AudioSource PlaySound(string soundName, Vector3 position = default, bool is3D = false)
    {
        Debug.Log($"PlaySound requested for: '{soundName}', Position: {position}, Is3D: {is3D}");

        // Check if sound exists
        if (!soundEffectDictionary.TryGetValue(soundName, out SoundEffect sound))
        {
            Debug.LogError($"Sound effect '{soundName}' not found in dictionary!");
            return null;
        }

        // Check if the clip is assigned
        if (sound.clip == null)
        {
            Debug.LogError($"Sound effect '{soundName}' has a null AudioClip assigned in the Inspector!");
            return null;
        }
        ...
        if (muteAllSounds)
        {
            Debug.LogWarning($"Skipping sound '{soundName}' because muteAllSounds is true.");
            return null;
        }
        ...
        if (masterVolume <= 0f)
        {
            Debug.LogWarning($"Skipping sound '{soundName}' because masterVolume is {masterVolume}.");
            return null;
        }

        // Check sound effect's specific volume
        if (sound.volume <= 0f)
        {
            Debug.LogWarning($"Skipping sound '{soundName}' because its specific volume is {sound.volume}.");
            return null;
        }
        ...
        if (audioSource == null)
        {
            Debug.LogError($"Could not get an available AudioSource from the pool for '{soundName}'!");
            return null;
        }
        
        Debug.Log($"Got AudioSource: {audioSource.gameObject.name} for sound '{soundName}'. Current state: isPlaying={audioSource.isPlaying}, clip={audioSource.clip?.name}");
        ...
        float randomPitch = sound.randomPitchVariation > 0 ? Random.Range(-sound.randomPitchVariation, sound.randomPitchVariation) : 0f;
        audioSource.pitch = sound.pitch + randomPitch;
        
        audioSource.loop = sound.loop; // Use the loop setting from the SoundEffect definition
        ...
        if (is3D)
        {
            ...
            audioSource.rolloffMode = AudioRolloffMode.Linear; // Or configure as needed
            audioSource.transform.position = position;
            Debug.Log($"Configuring '{soundName}' as 3D sound at {position}. SpatialBlend: {audioSource.spatialBlend}, MaxDistance: {audioSource.maxDistance}");
        }
        else
        {   
            //Ensure it's 2D if not specified otherwise
            audioSource.spatialBlend = 0f; 
            audioSource.transform.position = Instance.transform.position; // Play 2D sound from manager's position
            Debug.Log($"Configuring '{soundName}' as 2D sound. SpatialBlend: {audioSource.spatialBlend}");
        }

        Debug.Log($"Final AudioSource config for '{soundName}': Clip='{audioSource.clip.name}', Volume={audioSource.volume}, Pitch={audioSource.pitch}, Loop={audioSource.loop}, SpatialBlend={audioSource.spatialBlend}");

        // Play the sound
        try
        {
            Debug.Log($"Attempting to call Play() on AudioSource {audioSource.gameObject.name} for '{soundName}'...");
            audioSource.Play();
            Debug.Log($"Play() called successfully for '{soundName}'. AudioSource isPlaying: {audioSource.isPlaying}");
        }
        catch (System.Exception e)
        {
            Debug.LogError($"Error calling Play() for '{soundName}' on {audioSource.gameObject.name}: {e.Message}", audioSource.gameObject);
            return null; // Return null if Play failed
        }
    ...
    {
        Debug.Log($"Stopping all sounds, found {audioSourcePool.Count} sources");
        
        int stoppedCount = 0;
        // Stop all audio sources in the pool
        foreach (AudioSource source in audioSourcePool)
        {
        ...
        Debug.Log($"Stopped {stoppedCount} playing sounds");
        
        // Handle background sounds
        foreach (var kvp in activeBackgroundSounds)
        {
            if (kvp.Value != null && kvp.Value.isPlaying)
            {
                Debug.Log($"Stopping background sound: {kvp.Key}");
                kvp.Value.Stop();
            }
        }
    }
    ...
    {
        Debug.Log($"SetMute called with: {mute}, previous state: {muteAllSounds}");
        
        // Only take action if the state is changing
        if (mute == muteAllSounds)
        {
            Debug.Log("Mute state unchanged, taking no action");
            return;
        }
        
        muteAllSounds = mute;
        
        // If muting, stop all playing sounds
        if (muteAllSounds)
        {
            Debug.Log("Muting all sounds - stopping all currently playing sounds");
            StopAllSounds(); // This stops one-shots and background sounds
        }
        else
        {
            Debug.Log("Unmuting sounds - allowing new sounds to play");
            
            // NOTE: We don't automatically restart sounds here.
            // GameManager.TryStartAmbientSound handles the main ambient track.
            // Other systems should request their sounds again if needed.
            
            // This loop is okay for potentially resuming other specific background sounds
            // if they were managed differently, but the main ambient is handled by GameManager.
            /*
            foreach (var kvp in activeBackgroundSounds)
            {
                if (kvp.Value != null && !kvp.Value.isPlaying) // Check if not already playing
                {
                    // Check if this sound *should* be playing (e.g., based on game state)
                    // For now, we only explicitly restart the main ambient via GameManager
                    // Debug.Log($"Restarting background sound: {kvp.Key}");
                    // kvp.Value.Play();
                }
            }
            */
