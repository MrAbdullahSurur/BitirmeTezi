...
public class GameManager : NetworkBehaviour
{
    ...
    private void ShowGameOver()
    {
        ...
        {   
            // Find the specific GameOverText object by name instead of getting first TMP_Text
            Transform gameOverTextTransform = _gameOverPanel.transform.Find("GameOverText");
            if (gameOverTextTransform != null)
            {
                // Ensure the GameOverText object is active
                if (!gameOverTextTransform.gameObject.activeSelf)
                {
                    gameOverTextTransform.gameObject.SetActive(true);
                    Debug.Log("Activated GameOverText object that was inactive");
                }
                _gameOverText = gameOverTextTransform.GetComponent<TextMeshProUGUI>();
            }

            if (_gameOverText == null) 
            {
                Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Could not find GameOverText object within GameOverPanel!", _gameOverPanel);
            }
        }
