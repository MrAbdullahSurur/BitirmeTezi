...
public class GameManager : NetworkBehaviour
{
    ...
    private void ShowGameOver()
    {
        ...
        {   
            // Search for the GameOverText component within the panel, including inactive objects
            TextMeshProUGUI foundText = null;
            var allTexts = _gameOverPanel.GetComponentsInChildren<TextMeshProUGUI>(true); // Search inactive
            foreach (var textComponent in allTexts)
            {
                if (textComponent.gameObject.name == "GameOverText")
                {
            ...
            if (foundText != null)
            {
                _gameOverText = foundText;
                // Ensure the found object and its component are active
                if (!_gameOverText.gameObject.activeSelf)
                {
                    _gameOverText.gameObject.SetActive(true);
                    Debug.Log($"Activated the inactive {_gameOverText.gameObject.name} GameObject.", _gameOverText.gameObject);
                }
                if (!_gameOverText.enabled)
                {
                     _gameOverText.enabled = true; // Also ensure the component itself is enabled
                     Debug.Log($"Enabled the inactive {_gameOverText.gameObject.name} component.", _gameOverText);
                }
                 Debug.Log($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Found GameOverText component dynamically (possibly inactive). Object: {_gameOverText.gameObject.name}", _gameOverText.gameObject);

            }
            else
            {
                 Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Could not find GameOverText object named 'GameOverText' within GameOverPanel, even searching inactive!", _gameOverPanel);
            }
        }
        ...
        {
             // Ensure GameOverText object is active before trying to set its text
             if (!_gameOverText.gameObject.activeSelf)
             {
                 _gameOverText.gameObject.SetActive(true);
                 Debug.Log("Re-activated GameOverText object that became inactive");
             }
