...
public class GameManager : NetworkBehaviour
{
    ...
    {
        // HER ÇAĞRILDIĞINDA UI ELEMANLARINI BUL:
        _gameOverPanel = GameObject.FindGameObjectWithTag("GameOverPanel"); 
        if (_gameOverPanel == null) 
        {
             Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Could not find GameOverPanel GameObject! Ensure it exists in the scene and has the 'GameOverPanel' tag.");
             return; 
        }
        
        _gameOverText = _gameOverPanel.GetComponentInChildren<TMP_Text>(true); // true: deaktif alt objeleri de ara
        if (_gameOverText == null) 
        {
            // ExitButton altındaki text'i bulmayı deneyelim (Alternatif)
            if (_exitButton != null) { _gameOverText = _exitButton.GetComponentInChildren<TMP_Text>(true); }

            if (_gameOverText == null) {
                 Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Could not find TMP_Text component within GameOverPanel or ExitButton! Assign it correctly.", _gameOverPanel);
                 return; // Text bulunamadıysa devam etme
            }
            else {
                 Debug.LogWarning($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Found TMP_Text under ExitButton. Consider restructuring UI or assigning the correct reference directly.", _gameOverText.gameObject);
            }
        }
        // ---- Bulma Sonu ----
        
        Debug.Log($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Using found _gameOverText reference. Object: {_gameOverText.gameObject.name}", _gameOverText.gameObject);
        
        // Panelin aktif olduğundan emin ol
        if (!_gameOverPanel.activeSelf) { _gameOverPanel.SetActive(true); }
        
        // Skor kontrolcüsünü al (güvenlik kontrolü)
        if (_scoreController == null)
        {
            _scoreController = GetComponent<ScoreController>(); 
        }
        ...
        if (_scoreController != null && playerCount > 1)
        {
            ...
            int yourScore = _scoreController.GetPlayerScore(localClientId);
            int opponentScore = 0;
        ...
        else if (_scoreController != null && playerCount <= 1)
        {
