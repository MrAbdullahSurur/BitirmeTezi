...
public class GameManager : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        // Subscribe to the server started event to play music
    ...
    public override void OnNetworkDespawn()
    {
        ...
        if (NetworkManager.Singleton != null)
        {
            NetworkManager.Singleton.OnClientConnectedCallback -= OnClientConnected;
            NetworkManager.Singleton.OnClientDisconnectCallback -= OnClientDisconnected;
            // Unsubscribe from server started event
            NetworkManager.Singleton.OnServerStarted -= HandleServerStarted;
        }
    }
    ...
    private void ShowGameOver()
    {
        ...
        if (_gameOverPanel != null)
        {
            ...
            {
                // Get ScoreController reference if missing
                if (_scoreController == null)
                {
                    _scoreController = FindObjectOfType<ScoreController>();
                }
                ...
                if (_scoreController != null && playerCount > 1)
                {
                    ...
                    int yourScore = _scoreController.Score;
                    
                    // Get opponent's score (if there are only two players)
                    int opponentScore = 0;
                    ...
                    foreach (var entry in allScores)
                    {
                        if (entry.Key != localClientId)
                        {
                            ...
                            break; // Just get the first opponent's score
                        }
                    }
                    ...
                    Debug.Log($"Game Over - Local player {(isWinner ? "won" : "lost")} with score {yourScore}");
                }
                else if (_scoreController != null && playerCount == 1)
                {
                    // Single player mode - just show Game Over and score
                    int finalScore = _scoreController.Score;
                    _gameOverText.text = $"Oyun Bitti\nPuan: {finalScore}";
                    
                    Debug.Log($"Game Over - Single player with score {finalScore}");
                }
                else
                {
                    // Fallback if no score controller or something went wrong
                    _gameOverText.text = "Oyun Bitti";
                    Debug.LogWarning("ScoreController not found at game over, showing generic message");
                }
            }
            else
            {
    ...
    {
        // This method is called only on the server when it starts
        Debug.Log("GameManager: Server has started via OnServerStarted event. Attempting to play background music.");
        ...
        {
            SoundEffectManager.Instance.PlaySound("BackgroundMusic");
        }
        else
        {
             if (SoundEffectManager.Instance == null)
             {
                 Debug.LogError("GameManager (HandleServerStarted): SoundEffectManager.Instance is null when trying to play music!");
             }
             else
             {
                Debug.LogWarning("GameManager (HandleServerStarted): SoundEffect 'BackgroundMusic' not found in the SoundEffectManager list!");
             }
        }
    }
}