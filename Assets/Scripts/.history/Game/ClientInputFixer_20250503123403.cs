...
public class ClientInputFixer : MonoBehaviour
{
    ...
    private void FindAndFixClientPlayers()
    {
        ...
        foreach (var player in players)
        {
            ...
            // Check if this is a client-owned player
            if (player.IsOwner && !player.IsHost)
            {
                isClientPlayer = true;
            }
            // On host machine, check for client players by their color (green)
            else if (NetworkManager.Singleton.IsHost)
            {
                // Find sprite renderer to check color
                var spriteRenderer = player.GetComponentInChildren<SpriteRenderer>();
                if (spriteRenderer != null && 
                    spriteRenderer.color.g > 0.9f && // High green value
                    spriteRenderer.color.r < 0.3f)   // Low red value (to detect green)
                {
                    // This is the green player (client)
                    isClientPlayer = true;
                    Debug.Log("Found client player by color (green)");
                }
            }
    ...
    private void Update()
    {
        ...
        // Periodically check for client players that need fixing
        if (Time.frameCount % 300 == 0) // Every ~5 seconds at 60 fps
        {
            FindAndFixClientPlayers();
        }
    }
