...
public class GameManager : NetworkBehaviour
{
    ...
    {
        // Paneli bul ve aktif et (eğer null veya inaktifse)
        if (_gameOverPanel == null) 
        {
        ...
        // ---- GameOverText Referansını KONTROL ET (Bulmaya çalışma) ----
        if (_gameOverText == null) 
        {   
            // Eğer Inspector'dan atanmadıysa HATA VER ve çık.
             Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): _gameOverText is NULL! Please assign the correct TMP_Text component to the 'Game Over Text' field in the GameManager Inspector.", this);
             return; 
        }
        // ---- Kontrol Sonu ----
        
        Debug.Log($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): Using assigned _gameOverText reference. Object: {_gameOverText.gameObject.name}", _gameOverText.gameObject);
