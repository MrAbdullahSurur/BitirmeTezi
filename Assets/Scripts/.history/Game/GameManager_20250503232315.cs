...
public class GameManager : NetworkBehaviour
{
    ...
    private void ShowGameOver()
    {
        ...
        {   // Panelin altındaki TextMeshPro - Text (UI) bileşenini bul
            _gameOverText = _gameOverPanel.GetComponentInChildren<TMP_Text>(true); // true: deaktif olanları da ara
            if (_gameOverText == null) 
            {
        ...
        if (_gameOverText == null)
        {
             Debug.LogError($"GameManager ShowGameOver ({NetworkManager.Singleton?.LocalClientId ?? ulong.MaxValue}): _gameOverText is NULL when ShowGameOver is called!", this);
        }
        else
        {
