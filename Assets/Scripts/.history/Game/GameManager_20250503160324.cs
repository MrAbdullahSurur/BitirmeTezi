...
public class GameManager : NetworkBehaviour
{
    ...
    private void ShowGameOver()
    {
        ...
        if (_gameOverPanel != null)
        {
            ...
            {
                // Use the ScoreUI reference to get the latest score
                if (_scoreUI != null)
                {
                    // Get the score directly from the ScoreUI component
                    int finalScore = _scoreUI.GetCurrentScore();
                    string scoreText = _scoreUI.GetScoreText();
                    Debug.Log($"Getting final score from ScoreUI: {finalScore} (display text: {scoreText})");
                    
                    // Update game over text with the current displayed score
                    _gameOverText.text = $"Game Over\n{scoreText}";
                    
                    Debug.Log($"Set GameOverText to: {_gameOverText.text}");
                }
                // Fallback to ScoreController if ScoreUI is not assigned
                else if (_scoreController != null)
                {
                    // Get the current score from the NetworkVariable
                    int finalScore = _scoreController.Score;
                    Debug.Log($"ScoreUI not found. Falling back to ScoreController. Final score: {finalScore}");
                ...
                {
                    Debug.LogWarning("Neither ScoreUI nor ScoreController found, showing score 0");
                    _gameOverText.text = "Game Over\nScore: 0";
                }
            }
            else
