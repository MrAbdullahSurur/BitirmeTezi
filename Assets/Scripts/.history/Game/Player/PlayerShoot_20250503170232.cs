...
{
    [SerializeField]
    private GameObject _bulletPrefab; // Eski prefab referansı (geriye uyumluluk için)

    [Header("Host ve Client için farklı mermiler")]
    [SerializeField]
    private GameObject _hostBulletPrefab; // Host mermisi

    [SerializeField]
    private GameObject _clientBulletPrefab; // Client mermisi
    ...
    public override void OnNetworkSpawn()
    {
        ...
        // Backward compatibility - if specific prefabs are not assigned, use the generic one
        if (_hostBulletPrefab == null) _hostBulletPrefab = _bulletPrefab;
        if (_clientBulletPrefab == null) _clientBulletPrefab = _bulletPrefab;
    ...
    private void RequestShootServerRpc(Vector3 spawnPosition, Quaternion spawnRotation)
    {
        ...
        // Host için uygun mermiyi oluştur - true: host mermisi
        SpawnBullet(spawnPosition, spawnRotation, OwnerClientId, true);
    }
    ...
    public void ServerControlledFire()
    {
        ...
        if (timeSinceLastFire >= _timeBetweenShots)
        {
            ...
            if (_gunOffset != null)
            {
                 // Client için uygun mermiyi oluştur - false: client mermisi
                 SpawnBullet(_gunOffset.position, transform.rotation, OwnerClientId, false);
                 _lastFireTime = Time.time;
    ...
    private void SpawnBullet(Vector3 spawnPosition, Quaternion spawnRotation, ulong ownerClientId, bool isHostBullet)
    {
        if (!IsServer) return;

        // Uygun mermi prefabını seç
        GameObject bulletPrefabToUse = isHostBullet ? _hostBulletPrefab : _clientBulletPrefab;

        if (bulletPrefabToUse == null) {
             Debug.LogError($"Bullet Prefab not assigned! isHostBullet: {isHostBullet}", this);
             return;
        }

        GameObject bulletInstance = Instantiate(bulletPrefabToUse, spawnPosition, spawnRotation);
        ...
        if (networkObject != null)
        {
             ...
             if (bulletComponent != null)
             {
                 ...
                 Debug.Log($"Spawned {(isHostBullet ? "Host" : "Client")} bullet for player {ownerClientId}");
             }
