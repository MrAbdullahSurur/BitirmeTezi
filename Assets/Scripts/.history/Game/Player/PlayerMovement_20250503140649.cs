...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsOwner)
        {
            ...
            // Set the camera target for the local player
            SetCameraTarget();
    ...
    */
    ...
    // Add new method to set camera target
    private void SetCameraTarget()
    {
        if (!IsOwner) return; // Only the owner controls its camera

        // Ensure _camera reference is set (might have been set in PreventPlayerGoingOffScreen)
        if (_camera == null)
        {
            _camera = Camera.main;
        }
        ...
        {
            // -- Added Logging --
            Debug.Log($"Player {OwnerClientId}: Found Camera object named '{_camera.gameObject.name}' with InstanceID {_camera.gameObject.GetInstanceID()}", _camera.gameObject);
            ...
            {
                 Debug.LogWarning($"Player {OwnerClientId}: GetComponent<CameraFollow>() returned NULL on camera '{_camera.gameObject.name}'. Checking components manually...", _camera.gameObject);
                 // Let's list all components on the camera to be sure
                ...
                // The original warning:
                 Debug.LogWarning($"Player {OwnerClientId} could not find CameraFollow script on the Main Camera.", _camera);
            }
            // -- End Added Logging --
            else
            {
                cameraFollow.SetTarget(this.transform);
                Debug.Log($"Player {OwnerClientId} successfully set CameraFollow target on camera '{_camera.gameObject.name}'.", this);
            }
        }
        else
        {
            Debug.LogError($"Player {OwnerClientId} could not find the Main Camera.", this);
        }
    }
}
