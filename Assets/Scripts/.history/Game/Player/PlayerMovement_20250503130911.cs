...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private void HandleHostInputForClient()
    {
        ...
        if (clientMoveInput.magnitude > 1f) clientMoveInput.Normalize();
    ...
    public void ServerControlledMoveAndRotate(Vector2 moveInput)
    {
        ...
        if (moveInput != Vector2.zero)
        {
            ...
            // Calculate the target angle (z-axis)
            float targetAngle = targetRotation.eulerAngles.z;
            // Calculate the new angle using Mathf.MoveTowardsAngle
        ...
        // Update NetworkVariables for clients
        _networkPosition.Value = _rigidbody.position;
        _networkRotation.Value = Quaternion.Euler(0, 0, _rigidbody.rotation); // Convert float angle to Quaternion
        
        // Update animator on server instance
    ...
    private void FixedUpdate()
    {
        ...
        { 
             ...
             // Update network vars for host's own movement
    ...
    {
        // Sadece yerel oyuncunun UI'ını bağla
        if (!IsOwner) return; 
        
        // HealthBar UI
        HealthBarUI healthBar = FindObjectOfType<HealthBarUI>(); // Sahnedeki tek can barını bul varsayımı
        HealthController healthController = GetComponent<HealthController>();
        if (healthBar != null && healthController != null)
        {
            healthBar.SetTrackedHealthController(healthController);
            Debug.Log($"Local Player {OwnerClientId} connected its HealthController to HealthBarUI.");
        }
        else
        {
            Debug.LogWarning($"Local Player {OwnerClientId} could not find HealthBarUI or HealthController to connect.");
        }
        
        // Score UI bağlantısı ScoreController tarafından yapılıyor.
        // ScoreController scoreController = FindObjectOfType<ScoreController>();
        // ScoreUI scoreUI = FindObjectOfType<ScoreUI>();
        // if(scoreUI != null && scoreController != null) scoreUI.UpdateScore(scoreController);
    }
}
