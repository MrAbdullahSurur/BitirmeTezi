...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // --- Owner Logic Below ---
        ReadInput(); // Read input directly
        ApplyMovement(); 
        SetAnimation(_movementInput != Vector2.zero); 
    }

    // Read input directly from the action
    private void ReadInput()
    {
        if (_playerInput != null && _playerInput.enabled)
        {
             try
             {
                 // Get the currently active map's "Move" action
                 InputAction moveAction = _playerInput.currentActionMap?.FindAction("Move");
                 if (moveAction != null)
                 {
                     _movementInput = moveAction.ReadValue<Vector2>();
                     // Log if input detected
                     // if (_movementInput != Vector2.zero) Debug.Log($"Owner {OwnerClientId} ReadInput value: {_movementInput} from map: {_playerInput.currentActionMap?.name}");
                 }
                 else
                 {
                      _movementInput = Vector2.zero;
                      // Log error if action not found in active map
                      // if (_playerInput.currentActionMap != null) Debug.LogError($"'Move' action not found in active map '{_playerInput.currentActionMap.name}'");
                 }
             }
             catch (System.Exception e)
             {
                  Debug.LogError($"Error reading input action: {e.Message}");
                  _movementInput = Vector2.zero;
             }
        }
        else
