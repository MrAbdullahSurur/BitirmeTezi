...
public class ClientInputHandler : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        // Disable any PlayerInput component
        var playerInput = GetComponent<UnityEngine.InputSystem.PlayerInput>();
        if (playerInput != null)
        {
            Debug.Log("Disabling PlayerInput component in favor of direct input handling", this);
    ...
    private void FixedUpdate()
    {
        ...
        if (_animator != null)
        {
            _animator.SetBool("IsMoving", _currentMovement.sqrMagnitude > 0.01f);
        }
    }
    ...
    private void HandleMovementInput()
    {
        ...
        // Send movement to PlayerMovement script for smooth visual movement
        if (_playerMovement != null)
        {
            // Try to find and set _movementInput field using reflection
            var movementInputField = _playerMovement.GetType().GetField("_movementInput", 
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                
            if (movementInputField != null)
            {
                movementInputField.SetValue(_playerMovement, _currentMovement);
            }
        }
    ...
    private void HandleFireInput()
    {
        ...
        if (spacePressed && !_fireButtonDown)
        {
            ...
            _fireThisFrame = true;
            Debug.Log("Client: Space key pressed for fire", this);
        }
                ...
                if (_playerShoot != null)
                {
                    ...
                    if (shootMethod != null)
                    {
                        ...
                        if (gunOffset != null)
                        {
                            ...
                            // Also set firing fields in PlayerShoot to ensure visual updates
                            var fireContinuouslyField = _playerShoot.GetType().GetField("_fireContinuously", 
                                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                            var fireSingleField = _playerShoot.GetType().GetField("_fireSingle", 
                                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                                
                            if (fireContinuouslyField != null) fireContinuouslyField.SetValue(_playerShoot, _fireButtonDown);
                            if (fireSingleField != null) fireSingleField.SetValue(_playerShoot, _fireThisFrame);
                        }
                        else
                        {
