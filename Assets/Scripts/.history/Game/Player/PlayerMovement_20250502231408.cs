...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsOwner)
        {
            ...
            // Önce doğru map'i ayarla, sonra etkinleştir
            if (IsHost)
            {
                _playerInput.defaultActionMap = "PlayerWASD"; // Varsayılanı ayarla
                _playerInput.SwitchCurrentActionMap("PlayerWASD"); // Aktif olanı da değiştir (emin olmak için)
            }
            else 
            {
                 _playerInput.defaultActionMap = "PlayerArrows"; // Varsayılanı ayarla
                 _playerInput.SwitchCurrentActionMap("PlayerArrows"); // Aktif olanı da değiştir
            }
            _playerInput.enabled = true; // PlayerInput bileşenini şimdi etkinleştir
