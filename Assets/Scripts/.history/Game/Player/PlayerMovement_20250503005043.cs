...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        else // No need for !IsHost check, since we returned if !IsOwner
        {
            ...
            if (_smoothedMovementInput.sqrMagnitude > 0.001f)
            {
                ...
                // 4. Request server sync periodically (ALWAYS SYNC - removed frame count check)
    ...
    private void ClientStartup()
    {
        ...
        {
            _playerInput.enabled = true;
            // Make sure the action map name exactly matches the one in your Input Actions asset
            string clientActionMap = "PlayerArrows"; 
            try 
            {
                 _playerInput.SwitchCurrentActionMap(clientActionMap);
                 Debug.Log($"Client {OwnerClientId}: Switched PlayerInput to action map: {clientActionMap}");
            }
            catch (System.Exception e)
            {
                 Debug.LogError($"Client {OwnerClientId}: FAILED to switch to action map '{clientActionMap}'. Error: {e.Message}", this);
                 // Optionally try the other map as a fallback or log available maps
                 // Debug.Log("Available action maps: " + string.Join(", ", _playerInput.actions.actionMaps.Select(m => m.name)));
            }
        }
        else
        {
            Debug.LogError($"Client {OwnerClientId}: PlayerInput component is null in ClientStartup!", this);
        }
