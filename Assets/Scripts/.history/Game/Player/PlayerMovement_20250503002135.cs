...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        else if (!IsHost && _movementInput != Vector2.zero)
        {
            ...
            Vector3 desiredPosition = transform.position + new Vector3(_movementInput.x, _movementInput.y, 0) * _speed * Time.fixedDeltaTime; // Use fixedDeltaTime
            
            // 2. Apply position locally for responsiveness
            transform.position = desiredPosition;
            
            // 3. Handle rotation locally
            Quaternion targetRotation = Quaternion.LookRotation(transform.forward, _movementInput);
            transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, _rotationSpeed * Time.fixedDeltaTime); // Use fixedDeltaTime
    ...
    private void Update()
    {
        // Only process rotation sync for client owner in Update (visuals only)
        if (IsOwner && !IsHost && _movementInput != Vector2.zero)
        {
            // Rotation is handled in FixedUpdate now, Update is mainly for input checks if needed
            // or other non-physics related tasks.
        }
