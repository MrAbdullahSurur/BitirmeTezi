...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsOwner)
        {
            ...
            {
                // Change sync mode for client authority
                networkRigidbody.InLocalSpace = true;
                Debug.Log($"Player {OwnerClientId}: NetworkRigidbody2D found and set to InLocalSpace", this);
            }
            else
            {
    ...
    private void SyncPositionToServerServerRpc(Vector3 position)
    {
        ...
        // For now, we'll just ensure position sync by updating transform
