...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsClient && !IsHost && IsOwner)
        {
            ...
            // Check if we should use direct input handling for client
            if (!GetComponent<ClientInputHandler>())
            {
                // Add ClientInputHandler for direct input processing
                var clientHandler = gameObject.AddComponent<ClientInputHandler>();
                clientHandler.enabled = true;
                Debug.Log($"Added ClientInputHandler to client player {OwnerClientId}", this);
                
                // Also add our fallback handler, just in case
                TryAddFallbackHandler();
            }
        }
    ...
    private IEnumerator ClientStartup()
    {
        ...
        yield return null; // Added yield return to make it a valid coroutine
    }
    ...
    private void ClearInputDevicePairings()
    {
        ...
        // For brute force approach, use alternative method to clear control scheme
        try
        {
            // As a fallback, try to clear control scheme differently
            // We can't directly set currentControlScheme as it's read-only
            
            // Force the Input System to update
            InputSystem.Update();
            
            // Alternative approach: switch to null action map first
            try {
                _playerInput.SwitchCurrentActionMap("");
            } catch {}
            
            Debug.Log("Attempted to clear control scheme using alternative method", this);
        }
        catch (System.Exception e)
        {
