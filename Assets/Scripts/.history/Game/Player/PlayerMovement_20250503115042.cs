...
public class PlayerMovement : NetworkBehaviour
{
    ...
    // Store references to the action maps
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsOwner)
        {
            ...
            // --- Action Map'i Bul ve Etkinleştir ---
            string mapNameToEnable = IsHost ? "PlayerWASD" : "PlayerArrows";
            _activeActionMap = inputActions.FindActionMap(mapNameToEnable, throwIfNotFound: true);
            if (_activeActionMap != null)
            {
                _activeActionMap.Enable();
                _moveAction = _activeActionMap.FindAction("Move", throwIfNotFound: true);
                Debug.Log($"Owner {OwnerClientId}: Enabled Action Map '{_activeActionMap.name}'");
                if (_moveAction == null) 
                {
                     Debug.LogError($"Owner {OwnerClientId}: Could not find 'Move' action in map '{_activeActionMap.name}'!");
                }
            }
            else
            {
                 Debug.LogError($"Owner {OwnerClientId}: Could not find Action Map '{mapNameToEnable}' in the InputActionAsset!");
            }
            // --- Action Map Etkinleştirme Sonu ---
            
            StartCoroutine(ActivateVisualsAfterFade()); 
        }
        else
        {
             // Sahip olmayanlar için network interpolasyon hedeflerini başlat
             _networkPositionTarget = transform.position;
             _networkRotationTarget = transform.rotation;
             // Ensure non-owner doesn't have active maps (safety check)
    ...
    public override void OnNetworkDespawn()
    {
        ...
        // Disable the action map when despawning
    ...
    private void FixedUpdate()
    {
        ...
        // --- Owner Logic Below ---
        ReadInput(); // Reads from _moveAction if map is enabled
    ...
    {
        if (_moveAction != null && _activeActionMap.enabled)
        {
