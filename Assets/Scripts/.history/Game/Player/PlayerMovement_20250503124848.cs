...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private IEnumerator FindClientPlayerReference()
    {
        while (!_foundClient)
        {
            ...
            foreach (var clientId in NetworkManager.Singleton.ConnectedClientsIds)
            {
                ...
                // Get the NetworkObject for the client
                if (NetworkManager.Singleton.SpawnManager.GetPlayerNetworkObject(clientId, out NetworkObject clientNetworkObject))
                {
    ...
    public void ServerControlledMoveAndRotate(Vector2 moveInput)
    {
        ...
        Vector3 newPosition = _rigidbody.position + (Vector2)moveDelta; // Use Rigidbody position

        // Apply rotation
        if (moveInput != Vector2.zero)
        {
            Quaternion targetRotation = Quaternion.LookRotation(Vector3.forward, moveInput); // Use Vector3.forward for 2D
            Quaternion newRotation = Quaternion.RotateTowards(_rigidbody.rotation, targetRotation.eulerAngles.z, _rotationSpeed * Time.fixedDeltaTime); // Use Rigidbody rotation
