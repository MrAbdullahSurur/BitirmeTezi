...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        // --- Sahip Taraflı Ayarlar ---
        if (IsOwner)
        {
            _camera = Camera.main; 
            if (_camera == null) { Debug.LogError("Sahip oyuncu için Main Camera bulunamadı!"); }
            
            // --- Kontrol Şeması Ayarı ---
            if (IsHost) // Eğer bu sahip Host ise
            {
                _playerInput.SwitchCurrentActionMap("PlayerWASD");
                Debug.Log("Kontrol şeması: PlayerWASD", this);
            }
            else // Eğer bu sahip Host değilse (yani katılan bir Client ise)
            {
                 _playerInput.SwitchCurrentActionMap("PlayerArrows");
                 Debug.Log("Kontrol şeması: PlayerArrows", this);
            }
            // --- Kontrol Şeması Ayarı Sonu ---
