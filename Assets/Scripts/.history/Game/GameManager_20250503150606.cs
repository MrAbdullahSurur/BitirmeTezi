...
public class GameManager : NetworkBehaviour
{
    ...
    {
        Debug.Log($"Dead players count updated: {previous} -> {current}. IsClient={IsClient}, IsServer={IsServer}, GameEnded={_gameEnded}");
        
        if (current >= 2 && !_gameEnded)
        {
            // Both clients and server should show game over screen
            Debug.Log("Both players are dead - showing game over screen");
            ShowGameOver();
        }
    }
    ...
    public void OnPlayerDied()
    {
        ...
        Debug.Log($"Player died! Dead players count: {_deadPlayersCount.Value}, IsServer={IsServer}, IsClient={IsClient}");
