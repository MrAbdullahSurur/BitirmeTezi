...
public class ScoreController : NetworkBehaviour
{
    ...
    private void UpdateScoreUI()
    {
        ...
        if (scoreUI != null)
        { 
            Debug.Log($"ScoreController (Instance: {NetworkObjectId}): Found ScoreUI instance: {scoreUI.gameObject.name}. Calling UpdateScore...");
            scoreUI.UpdateScore(this);
        }
        else
        {
            Debug.LogError($"ScoreController (Instance: {NetworkObjectId}): UpdateScoreUI failed to FindObjectOfType<ScoreUI>() on {(IsServer ? "Server" : "Client")}");
        }
    }
}
