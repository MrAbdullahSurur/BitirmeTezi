...
public class ScoreController : NetworkBehaviour
{
    ...
    public void AddScoreToPlayer(ulong playerId, int amount)
    {
        ...
        Debug.Log($"SERVER: Adding {amount} score to player {playerId}");
        
        PlayerScoreData newData = _playerScores.Value;
        
        if (newData.Scores.ContainsKey(playerId))
        {
            int oldScore = newData.Scores[playerId];
            newData.Scores[playerId] += amount;
            Debug.Log($"SERVER: Player {playerId} score updated from {oldScore} to {newData.Scores[playerId]}");
        }
        else
        {
            newData.Scores[playerId] = amount;
            Debug.Log($"SERVER: Player {playerId} first score set to {amount}");
        }
