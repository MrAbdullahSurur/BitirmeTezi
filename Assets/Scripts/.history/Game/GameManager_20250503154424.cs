...
public class GameManager : NetworkBehaviour
{
    ...
    private void CheckAllPlayersDead()
    {
        ...
        {
            if (!entry.Value) // If player is not dead
            {
                alivePlayersCount++;
            }
        }
        
        bool allPlayersDead = (alivePlayersCount == 0);
        
        Debug.Log($"Alive players: {alivePlayersCount}, Total connected: {_connectedPlayersCount.Value}, All dead? {allPlayersDead}");
        
        if (allPlayersDead)
        {
            Debug.Log($"All players are dead - showing game over screen");
